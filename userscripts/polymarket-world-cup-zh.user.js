...
// @version      1.1.0
// @description  Polymarket 中文增强：导航精简、体育只保留实时/足球、实时页只显示足球、联赛与队伍汉化
...
(() => {
  ...
  const ATTRS = ["aria-label", "placeholder", "title", "alt", "value"];
  ...
  const CODE_MAP = {
    ...
    RSA: "南非", KSA: "沙特", IRN: "伊朗", IRQ: "伊拉克",
    QAT: "卡塔尔", UAE: "阿联酋", CHN: "中国", IND: "印度",
    UKR: "乌克兰", CZE: "捷克", SRB: "塞尔维亚", GRE: "希腊",
    NZL: "新西兰", PAN: "巴拿马", CRC: "哥斯达黎加", JAM: "牙买加",
  };

  const NON_SOCCER_RE = /篮球|网球|棒球|板球|冰球|橄榄球|高尔夫|格斗|拳击|国际象棋|匹克球|长曲棍球|排球|电竞|美式足球|Wimbledon|UFC|MLB|NBA|NHL|NFL|WNBA|ATP|WTA|ITF|F1|Combat|Basketball|Tennis|Baseball|Cricket|Hockey|Football|Golf|Esports|Boxing|Chess|Pickleball|Lacrosse|Volleyball|Rugby|MMA/i;
  ...
  function isSportsArea() {
    return /\/sports(\/|$)/.test(path()) || /\/event\//.test(path());
  }
  function isLivePage() { return /\/sports\/live\/?$/.test(path()); }
  function isDeepTranslatePage() {
    return isSportsArea() || path() === "/" || path() === "/zh" || /\/combos/.test(path());
  }
  function hide(el) { if (el && el.style) el.style.display = "none"; }
  function show(el) { if (el && el.style && el.style.display === "none") el.style.display = ""; }
  function shouldSkip(el) { return !el || SKIP.test(el.tagName) || el.isContentEditable; }
  ...
  function translateText(raw) {
    ...
    for (let i = 0; i < EXACT_KEYS.length; i++) {
      const key = EXACT_KEYS[i];
    ...
    s = s
      ...
      .replace(/\s+(1H|2H)?\s*Totals\b/g, function (_, h) {
        return h === "1H" ? "上半场总进球" : h === "2H" ? "下半场总进球" : "总进球";
      })
      .replace(/\bMoneyline\b/g, "胜负线")
      .replace(/\bSpread\b/g, "让分")
      .replace(/\bPlayer Props\b/g, "球员盘口")
      .replace(/\bGame Props\b/g, "比赛盘口")
    ...
    s = s.replace(/\b([A-Za-z][A-Za-z .&'’-]{1,40}?)\s*([+-]\d+(?:\.\d+)?)\b/g, function (full, name, odds) {
      ...
      if (EXACT[up]) return EXACT[up] + " " + odds;
      return full;
    });

    s = s.replace(/\b([A-Za-z][A-Za-z .&'’-]{1,40}?)\s+(\d+)\s*-\s*(\d+)\s+([A-Za-z][A-Za-z .&'’-]{1,40}?)\b/g, function (full, a, x, y, b) {
      const aa = EXACT[a] || CODE_MAP[a.toUpperCase()] || a;
      const bb = EXACT[b] || CODE_MAP[b.toUpperCase()] || b;
      return aa + " " + x + " - " + y + " " + bb;
    });

    s = s.replace(/\b([A-Z]{2,4})\b/g, function (m) { return CODE_MAP[m] || EXACT[m] || m; });
    s = s
      .replace(/(买入|卖出)\s*是(?:es|e|s)\b/g, "$1 是")
      .replace(/(买入|卖出)\s*否o?\b/g, "$1 否")
      .replace(/([\u4e00-\u9fff]+)\s+角球/g, "$1角球");
    return s;
  }
  ...
  function collectTextNodes(root, out) {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      else if (n.nodeType === 1 && !shouldSkip(n)) collectTextNodes(n, out);
    }
  }
  ...
  function fixBuySellButton(el) {
    ...
    if (texts[0].nodeValue !== next) texts[0].nodeValue = next;
    for (let i = 1; i < texts.length; i++) if (texts[i].nodeValue) texts[i].nodeValue = "";
  }
  ...
  function translateTree(root) {
    if (!root) return;
    if (root.nodeType === 3) return translateNodeText(root);
  ...
  function closestRow(a) {
    return a.closest("div.group") || a.closest("li") || a.parentElement || a;
  }

  function forceExpand(container) {
    if (!container) return;
  ...
  function expandSoccerLeagues() {
    ...
    document.querySelectorAll('button[aria-label*="sub-items"]').forEach((btn) => {
      const aria = btn.getAttribute("aria-label") || "";
      const parentText = ((btn.parentElement && btn.parentElement.textContent) || "").replace(/\s+/g, " ");
      const text = aria + " " + parentText;
      const isSoccer = /足球|Soccer|World Cup|世界杯/i.test(text);
      if (NON_SOCCER_RE.test(text) && !isSoccer) {
        hide(closestRow(btn));
        return;
      }
      if (!isSoccer) return;
      ...
      for (let i = 0; i < 5 && p; i++) {
        p.querySelectorAll('[aria-hidden="true"], .rah-static--height-zero').forEach(forceExpand);
        p = p.parentElement;
      }
    });
  }
  ...
  function simplifySportsSidebar() {
    ...
    document.querySelectorAll('a[href*="/sports/"]').forEach((a) => {
      ...
      if (NON_SOCCER_CODES.has(code) || NON_SOCCER_RE.test(label)) {
        hide(closestRow(a));
      }
    });
  }
  ...
  function simplifyLiveFeed() {
    ...
    const roots = document.querySelectorAll('[data-testid="virtuoso-item-list"] [data-index], main a[href*="/sports/"]');
    roots.forEach((node) => {
      const hrefs = [];
      if (node.getAttribute && node.getAttribute("href")) hrefs.push(node.getAttribute("href"));
      if (node.querySelectorAll) node.querySelectorAll('a[href*="/sports/"]').forEach((l) => hrefs.push(l.getAttribute("href") || ""));
      if (!hrefs.length) return;
      const soccer = hrefs.some(isSoccerHref);
      const non = hrefs.some((h) => {
        const c = hrefCode(h);
        return c && c !== "live" && !isSoccerHref(h) && /\/sports\//.test(h);
      });
      if (non && !soccer) hide(node);
      else if (soccer) show(node);
    });

    document.querySelectorAll("h2,h3,h4,p,span,a").forEach((el) => {
      const t = (el.textContent || "").replace(/\s+/g, " ").trim();
      if (!t || t.length > 42) return;
      if (!/^(ATP|WTA|ITF|Wimbledon|MLB|NBA|NHL|NFL|UFC|WNBA|F1|MLC|T20|Cricket|Tennis|Baseball|Basketball|Hockey|Combat)$/i.test(t)) return;
      const card = el.closest("[data-index]") || el.closest("section") || el.parentElement;
      if (!card) return;
      const html = card.innerHTML || "";
      if (!/\/sports\/(soccer|world-cup|epl|lal|laliga|mls|ucl|uel|ucol|bra|mex|nor|swe|kor|sea|bun|bundesliga|fl1|ligue-1)/.test(html)) {
        hide(card);
      }
    });
  }
  ...
  function cleanupWorldCupExtras() {
    if (!/\/sports\/world-cup|\/event\/fifwc-/.test(path())) return;
    document.querySelectorAll('a[href*="tab=props"],a[href*="tab=bracket"],a[href*="tab=map"]').forEach(hide);
    const comments = document.querySelector("#commentsInner");
    hide(comments && comments.parentElement);
    let about = document.querySelector("#about");
    about = about && about.parentElement && about.parentElement.parentElement;
    while (about) { hide(about); about = about.nextElementSibling; }

    let rush = null;
    document.querySelectorAll("span").forEach((s) => {
      if (!rush && /^(Goal rush stacks|进球连串)$/.test((s.textContent || "").trim())) rush = s;
    });
    while (rush && rush.tagName !== "SECTION") rush = rush.parentElement;
    hide(rush);

    let build = document.querySelector('button[aria-label^="Build "]');
    while (build && build.tagName !== "SECTION") build = build.parentElement;
    hide(build);

    document.querySelectorAll("button").forEach((b) => {
      if (/^(进球|助攻|射门|Goals|Assists|Shots)$/.test((b.textContent || "").trim())) hide(b);
    });
  }
  ...
  function hideGlobalChrome() {
    ...
    document.querySelectorAll("h2").forEach((h) => {
      if (/^(所有盘口|All Markets)$/.test((h.textContent || "").trim())) {
        const wrap = h.parentElement && h.parentElement.parentElement && h.parentElement.parentElement.parentElement;
        hide(wrap);
      }
    });
  }
  ...
  function run() {
    simplifyTopNav();
    if (isDeepTranslatePage()) translateTree(document.body);
    if (isSportsArea()) {
      expandSoccerLeagues();
